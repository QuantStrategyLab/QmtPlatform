...
固定样本行情的 dry-run smoke，然后向统一管理端发布脱敏的 `disabled` / `dry_run` 状态。dry-run 已完成、
但被确定性风控拦截为空权重，也属于通过：平台与安全边界都在正常工作。它不表示 QMT 已部署、已连接 miniQMT、
已启用 paper 或实盘。预检或 dry-run 真正失败才会发布为 `attention`；任何状态都不能改变目标开关或提交订单。
